...
uses
  ...
  ExtCtrls, ComCtrls, iniFiles, lcltype, db;
...
begin
  ...
  try
    row := dmSqlQsl.PrepareQslExportRow;
    ...
    begin
      ...
      row.ParamByName('id_cqrlog_main').AsInteger := dmData.qCQRLOG.FieldByName('id_cqrlog_main').AsInteger;
      ...
      begin
        row.ParamByName('idcall').AsString  := dmUtils.GetIDCall(qsl_via);
        row.ParamByName('dxcc').AsString    := dmDXCC.id_country(row.ParamByName('idcall').AsString,
                                                                     dmData.qCQRLOG.FieldByName('qsodate').AsDateTime);
        row.ParamByName('qsl_via').AsString := qsl_via
      end
      else begin
        qsl_via := '';
        row.ParamByName('idcall').AsString  := dmUtils.GetIDCall(dmData.qCQRLOG.FieldByName('callsign').AsString);
        row.ParamByName('dxcc').AsString    := dmDXCC.id_country(row.ParamByName('idcall').AsString,
                                                                     dmData.qCQRLOG.FieldByName('qsodate').AsDateTime);
        row.ParamByName('qsl_via').AsString := qsl_via
      end;
      //if row.ParamByName('idcall').AsString = row.ParamByName('qsl_via').AsString;
      ...
      begin
        if chkRemoveSep.Checked then
          row.ParamByName('remarks').AsString  := Rep(dmData.qCQRLOG.FieldByName('remarks').AsString)
        else
          row.ParamByName('remarks').AsString  := dmData.qCQRLOG.FieldByName('remarks').AsString
      end
      else
        row.ParamByName('remarks').AsString  := edtRemarks.Text;
      ...
      row.ParamByName('qsodate').AsString    := dmData.qCQRLOG.FieldByName('qsodate').AsString;
      {$ELSE}
      row.ParamByName('qsodate').AsDateTime  := dmData.qCQRLOG.FieldByName('qsodate').AsDateTime;
      {$ENDIF}
      row.ParamByName('time_on').AsString    := dmData.qCQRLOG.FieldByName('time_on').AsString;
      row.ParamByName('time_off').AsString   := dmData.qCQRLOG.FieldByName('time_off').AsString;
      row.ParamByName('callsign').AsString   := dmData.qCQRLOG.FieldByName('callsign').AsString;
      {$IFDEF DARWIN}
      //macOS compiler bug: FPC 3.2.2 for aarch64 with -O2 miscompiles
      //TParam.AsFloat (silently stores 0); the Currency path avoids it
      row.ParamByName('freq').AsCurrency     := dmData.qCQRLOG.FieldByName('freq').AsCurrency;
      {$ELSE}
      row.ParamByName('freq').AsFloat        := dmData.qCQRLOG.FieldByName('freq').AsFloat;
      {$ENDIF}
      row.ParamByName('mode').AsString       := dmData.qCQRLOG.FieldByName('mode').AsString;
      row.ParamByName('rst_s').AsString      := dmData.qCQRLOG.FieldByName('rst_s').AsString;
      row.ParamByName('rst_r').AsString      := dmData.qCQRLOG.FieldByName('rst_r').AsString;
      row.ParamByName('name').AsString       := Rep(dmData.qCQRLOG.FieldByName('name').AsString);
      row.ParamByName('qth').AsString        := Rep(dmData.qCQRLOG.FieldByName('qth').AsString);
      row.ParamByName('qsl_s').AsString      := dmData.qCQRLOG.FieldByName('qsl_s').AsString;
      row.ParamByName('qsl_r').AsString      := dmData.qCQRLOG.FieldByName('qsl_r').AsString;
      row.ParamByName('iota').AsString       := dmData.qCQRLOG.FieldByName('iota').AsString;
      row.ParamByName('pwr').AsString        := dmData.qCQRLOG.FieldByName('pwr').AsString;
      row.ParamByName('loc').AsString        := Copy(dmData.qCQRLOG.FieldByName('loc').AsString, 1, 6);
      row.ParamByName('my_loc').AsString     := dmData.qCQRLOG.FieldByName('my_loc').AsString;
      row.ParamByName('award').AsString      := Rep(dmData.qCQRLOG.FieldByName('award').AsString);
      row.ParamByName('band').AsString       := dmData.qCQRLOG.FieldByName('band').AsString;
      row.ParamByName('qslmsg').AsString     := Rep(qsl_msg);
      row.ParamByName('prop_mode').AsString  := dmData.qCQRLOG.FieldByName('prop_mode').AsString;
      row.ParamByName('satellite').AsString  := dmData.qCQRLOG.FieldByName('satellite').AsString;
      row.ParamByName('contestname').AsString := dmData.qCQRLOG.FieldByName('contestname').AsString;
      row.ParamByName('stx').AsString        := dmData.qCQRLOG.FieldByName('stx').AsString;
      row.ParamByName('stx_string').AsString := dmData.qCQRLOG.FieldByName('stx_string').AsString;
      row.ParamByName('srx').AsString        := dmData.qCQRLOG.FieldByName('srx').AsString;
      row.ParamByName('srx_string').AsString := dmData.qCQRLOG.FieldByName('srx_string').AsString;
      dmSqlQsl.InsertQslExportRow;
      dmData.qCQRLOG.Next
    end
  finally
  ...
  begin
    ...
    begin
      if cqrini.ReadInteger('QSLExport', 'DateFormat',0) = 0 then
        Write(f,dmUtils.MyDateToStr(rows.FieldByName('qsodate').AsDateTime),C_SEP);
      if cqrini.ReadInteger('QSLExport', 'DateFormat',0) = 1 then
        Write(f,FormatDateTime('yyyy-mmm-dd',rows.FieldByName('qsodate').AsDateTime),C_SEP);
      if cqrini.ReadInteger('QSLExport', 'DateFormat',0) = 2 then
        Write(f,FormatDateTime('dd.mm.yyyy',rows.FieldByName('qsodate').AsDateTime),C_SEP);
      if cqrini.ReadInteger('QSLExport', 'DateFormat',0) = 3 then
        Write(f,FormatDateTime('dd mmm yyyy',rows.FieldByName('qsodate').AsDateTime),C_SEP);
      ...
      begin
        ...
        Write(f,FormatDateTime('yyyy/mm/dd',rows.FieldByName('qsodate').AsDateTime),C_SEP);
      ...
      begin
        sep := FormatSettings.DateSeparator;
        FormatSettings.DateSeparator:='/';
        Write(f,FormatDateTime('mm/dd/yyyy',rows.FieldByName('qsodate').AsDateTime),C_SEP);
        FormatSettings.DateSeparator:=sep;
      end;
    end
    else begin
    ...
    if cqrini.ReadBool('QSLExport', 'time_on', True) then
      Write(f,rows.FieldByName('time_on').AsString,C_SEP)
    else
        if chkKeepCsvStructure.Checked then
	    Write(f,C_SEP);
    if cqrini.ReadBool('QSLExport', 'time_off', True) then
      Write(f,rows.FieldByName('time_off').AsString,C_SEP)
    else
        if chkKeepCsvStructure.Checked then
	    Write(f,C_SEP);
    if cqrini.ReadBool('QSLExport', 'CallSign', True) then
      Write(f,rows.FieldByName('callsign').AsString,C_SEP)
    else
        if chkKeepCsvStructure.Checked then
	    Write(f,C_SEP);
    if cqrini.ReadBool('QSLExport', 'Mode', True) then
      Write(f,rows.FieldByName('mode').AsString,C_SEP)
    else
        if chkKeepCsvStructure.Checked then
	    Write(f,C_SEP);
    if cqrini.ReadBool('QSLExport', 'Freq', True) then
      Write(f,FormatFloat('0.0000;;',rows.FieldByName('freq').AsFloat),C_SEP)
    else
        if chkKeepCsvStructure.Checked then
	    Write(f,C_SEP);
    ...
    begin
      ...
      begin
        Write(f,rows.FieldByName('rst_s').AsString,C_SEP)
      end
      else begin
        rst_tmp := rows.FieldByName('rst_s').AsString + '   ';
    ...
    begin
      if not cqrini.ReadBool('QSLExport','SplitRST_R',False) then
      begin
        Write(f,rows.FieldByName('rst_r').AsString,C_SEP);
      end
      else begin
        rst_tmp := rows.FieldByName('rst_r').AsString + '   ';
        Write(f,rst_tmp[1],C_SEP);
        Write(f,rst_tmp[2],C_SEP);
        Write(f,rst_tmp[3],C_SEP);
      end;
    ...
    if cqrini.ReadBool('QSLExport', 'Name', True) then
      Write(f,rows.FieldByName('name').AsString,C_SEP)
    else begin
        if chkKeepCsvStructure.Checked then
	    Write(f,C_SEP);
    end;
    if cqrini.ReadBool('QSLExport', 'QTH', True) then
      Write(f,rows.FieldByName('qth').AsString,C_SEP)
    else begin
        if chkKeepCsvStructure.Checked then
	    Write(f,C_SEP);
    end;
    if cqrini.ReadBool('QSLExport', 'band', True) then
      Write(f,rows.FieldByName('band').AsString,C_SEP)
    else begin
        if chkKeepCsvStructure.Checked then
	    Write(f,C_SEP);
    end;
    if cqrini.ReadBool('QSLExport', 'Propagation', True) then
      Write(f,rows.FieldByName('prop_mode').AsString,C_SEP)
    else begin
        if chkKeepCsvStructure.Checked then
	    Write(f,C_SEP);
    end;
    if cqrini.ReadBool('QSLExport', 'Satellite',  True) then
      Write(f,rows.FieldByName('satellite').AsString,C_SEP)
    else begin
        if chkKeepCsvStructure.Checked then
	    Write(f,C_SEP);
    end;
    if cqrini.ReadBool('QSLExport', 'ContestName',  True) then
      Write(f, rows.FieldByName('contestname').AsString,C_SEP)
    else begin
        if chkKeepCsvStructure.Checked then
	    Write(f,C_SEP);
    end;
    if cqrini.ReadBool('QSLExport', 'QSL_S', True) then
      Write(f,rows.FieldByName('qsl_s').AsString,C_SEP)
    else begin
        if chkKeepCsvStructure.Checked then
	    Write(f,C_SEP);
    end;
    if cqrini.ReadBool('QSLExport', 'QSL_R', True) then
      Write(f,rows.FieldByName('qsl_r').AsString,C_SEP)
    else begin
        if chkKeepCsvStructure.Checked then
	    Write(f,C_SEP);
    end;
    if cqrini.ReadBool('QSLExport', 'QSL_VIA', True) then
      Write(f,rows.FieldByName('qsl_via').AsString,C_SEP)
    else begin
        if chkKeepCsvStructure.Checked then
	    Write(f,C_SEP);
    end;
    if cqrini.ReadBool('QSLExport', 'locator', True) then
      Write(f,rows.FieldByName('loc').AsString,C_SEP)
    else begin
        if chkKeepCsvStructure.Checked then
	    Write(f,C_SEP);
    end;
    if cqrini.ReadBool('QSLExport', 'MyLoc', True) then
      Write(f,rows.FieldByName('my_loc').AsString,C_SEP)
    else begin
        if chkKeepCsvStructure.Checked then
	    Write(f,C_SEP);
    end;
    if cqrini.ReadBool('QSLExport', 'Distance', True) then
      Write(f,frmMain.CalcQrb(rows.FieldByName('my_loc').AsString,rows.FieldByName('loc').AsString,False),C_SEP)
    else begin
        if chkKeepCsvStructure.Checked then
	    Write(f,C_SEP);
    end;
    if cqrini.ReadBool('QSLExport', 'IOTA', True) then
      Write(f,rows.FieldByName('iota').AsString,C_SEP)
    else begin
        if chkKeepCsvStructure.Checked then
	    Write(f,C_SEP);
    end;
    if cqrini.ReadBool('QSLExport', 'award', True) then
      Write(f,rows.FieldByName('award').AsString,C_SEP)
    else begin
        if chkKeepCsvStructure.Checked then
	    Write(f,C_SEP);
    end;
    if cqrini.ReadBool('QSLExport', 'power', True) then
      Write(f,rows.FieldByName('pwr').AsString,C_SEP)
    else begin
        if chkKeepCsvStructure.Checked then
	    Write(f,C_SEP);
    end;
    if cqrini.ReadBool('QSLExport', 'Remarks', True) then
      Write(f,rows.FieldByName('remarks').AsString,C_SEP)
    else begin
        if chkKeepCsvStructure.Checked then
	    Write(f,C_SEP);
    end;
    if cqrini.ReadBool('QSLExport', 'QSLMsg', True) then
      Write(f,rows.FieldByName('qslmsg').AsString,C_SEP)
    else begin
        if chkKeepCsvStructure.Checked then
	    Write(f,C_SEP);
    end;
    if cqrini.ReadBool('QSLExport', 'ContestNrS', True) then
      Write(f,rows.FieldByName('stx').AsString ,C_SEP)
    else begin
        if chkKeepCsvStructure.Checked then
	    Write(f,C_SEP);
    end;
    if cqrini.ReadBool('QSLExport', 'ContestMsgS', True) then
      Write(f,rows.FieldByName('stx_string').AsString,C_SEP)
    else begin
        if chkKeepCsvStructure.Checked then
	    Write(f,C_SEP);
    end;
    if cqrini.ReadBool('QSLExport', 'ContestNrR',  True) then
      Write(f,rows.FieldByName('srx').AsString ,C_SEP)
    else begin
        if chkKeepCsvStructure.Checked then
	    Write(f,C_SEP);
    end;
    if cqrini.ReadBool('QSLExport', 'ContestMsgR',  True) then
      Write(f,rows.FieldByName('srx_string').AsString ,C_SEP)
    else begin
        if chkKeepCsvStructure.Checked then
	    Write(f,C_SEP);
    end;
...
//  FieldCount := GetExpFieldCount;
  ...
  try  try
    Rewrite(f);
    rows := dmSqlQsl.OpenQslExportRows;

    while not rows.Eof do
    begin
      if chkMarkSent.Checked then
      begin
        qsl_s := rows.FieldByName('qsl_s').AsString;
        if Pos('S',qsl_s) = 1 then
          qsl_s := copy(qsl_s,2,Length(qsl_s)-1)
        else begin
          if qsl_s = '' then
          begin
            if rows.FieldByName('qsl_s').AsString <> '' then
              qsl_s := 'MB'
            else
              qsl_s := 'B'
          end
        ...
        dmSqlQsl.MarkQslSentFromLabels(qsl_s, dmUtils.DateInRightFormat(dmUtils.GetDateTime(0)),
                                       rows.Fields[2].AsInteger)
      end;

      if old <> rows.FieldByName('callsign').AsString then
      begin
        if (old <> '') then
        begin
      ...
      if lNr+1 > MaxQ then
        ...
        old := rows.FieldByName('callsign').AsString;
