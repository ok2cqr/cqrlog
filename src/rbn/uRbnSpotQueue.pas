...
{ Bounded, thread safe queue of spot lines between the socket and the worker.
...
type
  ...
  private
    ...
    FItems   : array of String;  //ring buffer
  ...
  public
    ...
    procedure Push(const Line : String);
    function  Pop(out Line : String) : Boolean;
...
procedure TRbnSpotQueue.Push(const Line : String);
begin
  ...
  try
    ...
    FItems[(FHead + FCount) mod Length(FItems)] := Line;
    Inc(FCount)
  finally
...
function TRbnSpotQueue.Pop(out Line : String) : Boolean;
begin
  Line := '';
  EnterCriticalsection(FCrit);
  try
